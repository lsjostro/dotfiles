...
let
  passage-wrapped = pkgs.writeShellScriptBin "passage" ''
    get_tpm_pin() {
      local key_id
      if key_id=$(${pkgs.keyutils}/bin/keyctl request user age-tpm-pin @s 2>/dev/null); then
        ${pkgs.keyutils}/bin/keyctl pipe "$key_id"
      else
        local pin
        pin=$(${pkgs.systemd}/bin/systemd-ask-password "TPM PIN:") || return 1
        key_id=$(echo -n "$pin" | ${pkgs.keyutils}/bin/keyctl padd user age-tpm-pin @s)
        ${pkgs.keyutils}/bin/keyctl timeout "$key_id" 3600
        echo -n "$pin"
      fi
    }
